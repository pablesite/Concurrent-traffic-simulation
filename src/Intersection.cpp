...
{
    std::lock_guard<std::mutex> lock(_mutexWV);
...
{
    std::unique_lock<std::mutex> lck(_mtxTerm);
    std::cout << "Intersection #" << _id << "::addVehicleToQueue: "
              << "Vehicle #" << vehicle->getID() << " thread id = " << std::this_thread::get_id() << std::endl;
    lck.unlock();
